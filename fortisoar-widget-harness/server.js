...
const { packageWidget, bumpVersion, isValidVersion, writeInfoVersion, syncSourceToInfoJson, validateWidget, suggestInfoFix, applyInfoFix, } = require("./packager");
...
const { resolveSoarEnv, resolveSoarEnvFile, listEnvFiles, isExplicitHostOverride } = require("./lib/soarEnv");
...
function applySoarEnvFile(file) {
    ...
    if (!match)
        throw new Error(`unknown or unusable env file: ${file}`);
    const resolved = resolveSoarEnvFile(path.join(__dirname, file));
    if (!resolved.host)
        throw new Error(`env file has no FSR_BASE_URL: ${file}`);
    HOST = resolved.host;
    USER = resolved.user;
    PASS = resolved.pass;
    ACTIVE_ENV = file;
...
let activeFixtureWidgetId = null;
...
const PLATFORM_SERVICES = Array.from(new Set([
    ...FSR_BUNDLE_SERVICES,
...
let HARNESS_STUBBED_SERVICES = (() => {
    try {
        return readHarnessStubbedServices();
    }
    catch (_a) {
        return [];
    }
...
let REGISTERED_SERVICES = Array.from(new Set([...PLATFORM_SERVICES, ...HARNESS_STUBBED_SERVICES]));
...
if (!HERMETIC) {
    fs.watch(HARNESS_MODULE_PATH, { persistent: false }, () => {
        try {
            ...
            REGISTERED_SERVICES = Array.from(new Set([...PLATFORM_SERVICES, ...HARNESS_STUBBED_SERVICES]));
            for (const w of WIDGETS)
                refreshWidget(w);
            broadcast({ type: "harness-reload", services: REGISTERED_SERVICES });
        }
        catch (e) {
            console.warn(`harness.module.js reload failed: ${e instanceof Error ? e.message : String(e)}`);
        }
    });
}
            ...
            if (e.isDirectory())
                ...
                try {
                    ...
                    for (const name of HU.parseRegisteredServices(js)) {
                        if (!(name in out))
                            out[name] = "widgetAssets/" + sub;
                    }
                }
                catch ( /* skip unreadable file */_b) { /* skip unreadable file */ }
            }
        }
    };
    walk(root, "");
...
function lintFor(widget) {
    ...
    catch ( /* surfaced below */_a) { /* surfaced below */ }
...
function widgetIsPublished(raw) {
    let parsed;
    try {
...
function upstreamRequest(opts) {
    return new Promise((resolve, reject) => {
        ...
        const req = mod.request({
            ...
            headers: Object.assign({ Accept: "*/*" }, opts.body ? { "Content-Length": Buffer.byteLength(opts.body) } : {}, opts.headers || {}),
        }, (res) => {
            let data = "";
...
function upstreamMultipart(opts) {
    return new Promise((resolve, reject) => {
...
async function ensureToken() {
    ...
    if (!tokenPromise) {
        ...
        tokenPromise = authenticate()
            ...
            console.log(`auth: ok, token expires ${new Date(tokenExpiry).toISOString()}`);
            return token;
        })
            .finally(() => {
            tokenPromise = null;
...
const WIDGETS_SRC = WIDGET_ROOTS.find((p) => path.basename(p) === "widgets-src") ||
    WIDGET_ROOTS[0] ||
...
function scanStaleVersionRefs(widgetDir, name, version) {
    ...
    for (const file of VERSIONED_REF_FILES) {
        ...
        if (seen.size > 0)
            out.push({ file, staleVersions: Array.from(seen) });
    }
    return out;
}
...
function scanCapabilities(widgetDir) {
    let monaco = false;
    let editors = false;
    const files = [];
    const walk = (dir) => {
    ...
    for (const f of files) {
        ...
        if (!monaco && MONACO_MARKERS.test(src))
            monaco = true;
        if (!editors && EDITOR_MARKERS.test(src))
            editors = true;
    }
...
function discoverWidgets() {
    ...
    for (const root of WIDGET_ROOTS) {
        for (const e of fs.readdirSync(root, { withFileTypes: true })) {
            if (!e.isDirectory() || seenFolders.has(e.name))
                continue;
...
function broadcast(obj) {
    ...
    for (const res of sseClients) {
        ...
        catch ( /* client gone */_a) { /* client gone */ }
    }
}
...
function refreshWidget(w) {
    ...
    catch ( /* lint will report */_a) { /* lint will report */ }
...
function recordProxy(entry) {
    ...
    if (PROXY_LOG.length > PROXY_LOG_MAX)
        PROXY_LOG.shift();
...
app.use("/harness.module.js", express.static(path.resolve(__dirname, "harness.module.js"), { etag: false, cacheControl: false }));
...
// /lib static handler.
app.get("/lib/harnessUtils.js", (_req, res) => {
    try {
...
app.use("/lib", express.static(path.resolve(__dirname, "lib"), { etag: false, cacheControl: false }));
...
app.use("/node_modules/monaco-editor", express.static(path.resolve(__dirname, "node_modules", "monaco-editor"), {
    etag: false,
    cacheControl: false,
}));
...
app.use("/app", express.static(path.join(FSR_SRC_DIR, "templates-extracted", "app"), {
    etag: false,
    cacheControl: false,
    fallthrough: true,
}));
...
function loadPatchedFsrApp() {
    ...
    const patched = src.replace(/angular\.module\("cybersponse",\s*\[[^\]]*\]\)/, `angular.module("cybersponse", ${depsLiteral})`);
    if (patched === src) {
...
function isLocalPath(p) {
    if (LOCAL_PATHS.has(p))
        return true;
    if (p.startsWith("/lib/"))
        return true;
    for (const w of WIDGETS) {
        if (p.startsWith(`/${w.id}/`))
            return true;
        if (p.startsWith(`/widgets/installed/${w.id}/`))
            return true;
    }
    if (p.startsWith("/_fsr/"))
        return true;
    return false;
}
...
app.post("/_fsr/active-widget", express.json(), (req, res) => {
...
app.post("/_fsr/soar-env", express.json(), (req, res) => {
    ...
    try {
        ...
        try {
            fs.writeFileSync(ACTIVE_ENV_STATE_FILE, file);
        }
        catch (_) { }
        let shown = host;
        try {
            shown = new URL(host).host;
        }
        catch (_) { }
...
app.get("/_fsr/hermetic-misses", (_req, res) => {
...
function readCacheEntry(url) {
    try {
        return JSON.parse(fs.readFileSync(cacheFileFor(url), "utf8"));
    }
    catch (_a) {
        return null;
    }
...
app.use(async (req, res, next) => {
    ...
    if (CACHE_MODE !== "cached")
        return next();
    if (!isCacheable(req.originalUrl))
        return next();
...
function attachWatcher(w) {
    ...
    try {
        fs.watch(w.dir, { persistent: false }, (_event, filename) => {
            if (!filename || !HOT_RELOAD_FILES.has(filename))
                return;
            if (debounce)
...
function registerImportedWidget(folder) {
    ...
    if (!w)
        throw new Error(`widgets-src/${folder} is not a valid widget`);
...
app.post("/_fsr/package/:id", express.json(), async (req, res) => {
    ...
    try {
        ...
        if (body.version != null && body.version !== "") {
            ...
            version = bumpVersion(version, String(body.bump));
        }
...
app.post("/_fsr/install/:id", express.json(), async (req, res) => {
    ...
    try {
        ...
        const preflight = validateWidget(w.dir, freshInfoForValidation);
        if (preflight.errors.length > 0) {
            console.warn(`install: validation failed for ${w.folder}: ${preflight.errors.join("; ")}`);
            return res.status(400).json({
                error: "widget validation failed",
                validation: preflight,
                suggestedFix: suggestInfoFix(freshInfoForValidation),
        ...
        async function widgetRecordByName(name) {
            ...
            if (r.status < 200 || r.status >= 300)
                return null;
            try {
                const m = (JSON.parse(r.body)["hydra:member"] || []);
        ...
        let devManifest = {};
        {
            ...
            if (dr.status >= 200 && dr.status < 300) {
                try {
                    ...
                    const member = (dj["hydra:member"] || [])[0];
        ...
        const publishPayload = Object.assign(Object.assign({}, devManifest), { "@id": `/api/3/widgets/${uuid}`, draft: false, installed: true, enablePublish: false, replace: true, replaceVersions: [], publishedDate: Math.floor(Date.now() / 1000) });
        ...
        for (let i = 0; i < 10; i++) {
            ...
            if (published && published.version === newVersion)
                break;
        }
        console.log(`install: published ${widgetName}; record=${published ? `${published.version}(draft=${published.draft})` : "absent"}`);
        ...
        const assetPath = `/widgets/installed/${freshInfo.name}-${freshInfo.version}/view.controller.js`;
        ...
        const looksLikeController = assetBody.includes(".controller(") || assetBody.includes("angular");
        if (assetRes.status < 200 || assetRes.status >= 300 || isSpaFallback || !looksLikeController) {
            return res.status(502).json({
                error: `publish reported success but asset files did NOT extract: ` +
...
app.get("/_fsr/remote-widgets", async (_req, res) => {
    try {
        ...
        try {
            ...
            return res.status(502).json({ error: "non-JSON upstream response" });
        }
        ...
        const widgets = members.map((w) => ({
            ...
            section: String((w.metadata && w.metadata.section) || w.section || ""),
...
function extractTgz(tgzPath, destDir) {
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => code === 0 ? resolve() : reject(new Error(`tar exit ${code}: ${stderr.trim()}`)));
    });
}
async function installTgzBuffer(tgzBuf, folderArg) {
    ...
    try {
        ...
        if (!fs.existsSync(infoPath)) {
            const err = new Error("tgz missing info.json");
            err.status = 400;
            throw err;
        }
        const info = JSON.parse(fs.readFileSync(infoPath, "utf8"));
        ...
        if (!folder) {
            const err = new Error("could not derive folder name");
            err.status = 400;
            throw err;
        }
        const dest = path.join(WIDGETS_SRC, folder);
        if (fs.existsSync(dest)) {
            const err = new Error(`widgets-src/${folder} already exists`);
            err.status = 409;
            throw err;
        }
...
app.post("/_fsr/upload-tgz", express.raw({ type: ["application/gzip", "application/x-gzip", "application/octet-stream"], limit: "100mb" }), async (req, res) => {
...
async function ensureAuthMiddleware(req, res, next) {
    ...
    try {
        ...
        console.error(`auth failed for ${req.method} ${req.originalUrl}: ${e instanceof Error ? e.message : String(e)}`);
...
function shouldCapture(req) {
    if (!PROXY_VERBOSE && !req.originalUrl.startsWith("/api/"))
        return false;
    return true;
}
...
if (HERMETIC) {
    ...
    app.get("/api/3/system_settings", (_req, res) => {
        try {
    ...
    app.get(/^\/api\/3\/([^/]+)\/([^/]+)$/, (req, res, next) => {
        const module = decodeURIComponent(req.params[0]);
        const id = decodeURIComponent(req.params[1]);
        if (RESERVED_API3_HEADS.has(module))
            return next(); // → loud HERMETIC-MISS
    ...
    // error on `.data`.
    const serveConnectors = (_req, res) => {
...
if (HERMETIC) {
    ...
    app.use((req, res, next) => {
        if (req.path.startsWith("/api/"))
            return ensureAuthMiddleware(req, res, next);
        next();
    });
    app.use(proxy);
}
if (require.main === module) {
    if (!HOST || !USER || !PASS) {
        console.error("Missing FSR_BASE_URL / FSR_USERNAME / FSR_PASSWORD. " +

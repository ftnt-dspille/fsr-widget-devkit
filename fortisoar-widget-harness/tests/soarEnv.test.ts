...
describe("isExplicitHostOverride", () => {
  const FILE = { FSR_BASE_URL: "https://192.0.2.205" };

  test("true when env host differs from the .env file host", () => {
    expect(isExplicitHostOverride({ FSR_BASE_URL: "https://192.0.2.159" }, FILE)).toBe(true);
  });
  test("false when env host equals the .env file host (mere dotenv copy)", () => {
    expect(isExplicitHostOverride({ FSR_BASE_URL: "https://192.0.2.205" }, FILE)).toBe(false);
  });
  ...
  test("legacy FORTISOAR_HOST is honored as the env host", () => {
    expect(isExplicitHostOverride({ FORTISOAR_HOST: "https://192.0.2.159" }, FILE)).toBe(true);
  });
  test("true when env host is set but the .env file declares none", () => {
    expect(isExplicitHostOverride({ FSR_BASE_URL: "https://192.0.2.159" }, {})).toBe(true);
  });
});

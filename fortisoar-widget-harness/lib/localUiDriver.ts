...
/** Harness UI context values -- the `#ctx` select in public/index.html. */
type HarnessContext = "dashboard" | "viewpanel" | "drawer" | "playbook";
...
export interface OpenLocalWidgetOpts {
  module?: string;
  recordUuid?: string;
  /**
   * A non-record surface. On a box this is a ui-router path; here it only
   * selects the harness mount CONTEXT, since there is no SPA to route. Paths
   * naming a playbook map to the playbook-editor context, anything else to the
   * dashboard. Pass `context` directly to be explicit.
   */
  mountPath?: string;
  /** Explicit harness mount context; wins over the mountPath heuristic. */
  context?: HarnessContext;
  /** Unsupported locally -- see the module docstring. Throws if set. */
  visitFirst?: string;
  headless?: boolean;
  /** Harness origin. Defaults to FSRPB_LOCAL_BASE or http://localhost:4401. */
  base?: string;
  /** Widget folder name to mount. */
  widgetName?: string;
  /** Widget config seeded into localStorage before mount. */
  config?: Record<string, unknown>;
  /** Hold one browser across rows; see the live driver for the trade-off. */
  reuse?: boolean;
}
 ...
 */
const DEFAULT_WIDGET_CONFIG: Record<string, unknown> = {
 ...
 */
async function preflight(base: string): Promise<HarnessInfo> {
  let info: HarnessInfo;
  try {
    const r = await fetch(`${base}/_fsr/info`);
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    info = await r.json() as HarnessInfo;
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    throw new Error(
      `local harness not reachable at ${base} (${msg}). Start it with the local ` +
      `connector wired in:\n  FSR_LOCAL_CONNECTOR=1 PORT=4401 node server.js\n` +
      `See LOCAL_DEV.md.`);
  }
  if (!info.localConnector) {
    throw new Error(
      `the harness at ${base} is running WITHOUT FSR_LOCAL_CONNECTOR=1, so ` +
...
async function resolveWidgetId(base: string, name: string): Promise<string> {
  const r = await fetch(`${base}/_fsr/widgets`);
  const data = await r.json() as { widgets?: { name: string; id: string }[] };
  const list = data.widgets || (data as unknown as { name: string; id: string }[]);
  const w = (list || []).find((x) => x.name === name);
  if (!w || !w.id) {
    throw new Error(
      `widget "${name}" is not installed in the harness at ${base}. ` +
 ...
 * live driver returns.
 */
export async function openWidgetDrawer(opts: OpenLocalWidgetOpts = {}): Promise<ChatSession> {
  if (opts.visitFirst) {
    throw new Error(
      "localUiDriver: `visitFirst` is not supported. It reproduces a stale-entity " +
  ...
  const ctx: HarnessContext = opts.context
  ...
  await page.addInitScript(({ id, ctx, mod, rec, cfg }) => {
    ...
    if (rec) localStorage.setItem("harness.id", rec);
    if (cfg) localStorage.setItem("harness:config:" + id, JSON.stringify(cfg));
  }, {
    id: widgetId, ctx, mod, rec: opts.recordUuid || "",
    cfg: (opts.config || DEFAULT_WIDGET_CONFIG) as Record<string, unknown>,
  });
  ...
  const mount = async () => {
    ...
    try {
      await page.waitForFunction(
        () => {
          const w = window as unknown as { __fortiaiAgenticAssistant__?: { state?: string } };
      ...
      const st = await page.evaluate(() => {
        const w = window as unknown as {
          __HARNESS_RENDER_STATE?: { phase?: string; lastError?: unknown };
          __HARNESS_RECORD?: unknown;
        };
        return {
          phase: w.__HARNESS_RENDER_STATE?.phase || "(none)",
          err: String(w.__HARNESS_RENDER_STATE?.lastError || ""),
          record: !!w.__HARNESS_RECORD,
        };
      }).catch(() => ({ phase: "(unreadable)", err: "", record: false }));
      throw new Error(
        `localUiDriver: the widget never mounted (render phase "${st.phase}", ` +
    ...
    const mocked = await page.evaluate(() => {
      const w = window as unknown as { __fortiaiAgenticAssistant__?: { mockActive?: boolean } };
      return !!(w.__fortiaiAgenticAssistant__ && w.__fortiaiAgenticAssistant__.mockActive);
    });
    if (mocked) {
      throw new Error(
        "localUiDriver: the widget booted in MOCK mode despite ?mode=real, so it would " +

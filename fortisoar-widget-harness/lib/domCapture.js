...
async function captureDom(page, rootSelector, styleProps = STYLE_PROPS, timeout = 5000) {
    ...
    const raw = await page.evaluate((args) => {
        ...
        const walk = (el, path, depth) => {
            ...
            for (const p of args.styleProps)
            ...
            if (kids.length > args.maxChildren)
                capsHit = true;
            const n = Math.min(kids.length, args.maxChildren);
            for (let i = 0; i < n; i++)
                walk(kids[i], path + "/" + i, depth + 1);
        };
        walk(root, "0", 0);
 ...
 *  away. */
function staticClasses(classes) {
...
function summarizeDomDiff(harnessDom, soarDom) {
    const notes = [];
    ...
    if (!tagMismatch) {
        ...
        for (const sn of soarDom.nodes) {
            ...
            if (hStatic.join(" ") !== sStatic.join(" ")) {
                identityDivergences.push(`${sn.path}: harness=${hn.tag}.${hStatic.join(".") || "(none)"} soar=${sn.tag}.${sStatic.join(".") || "(none)"}`);
                continue;
            }
        ...
        if (identityDivergences.length) {
            ...
            notes.push(`${identityDivergences.length} element-identity divergence(s) (different elements at same path — ` +
...
module.exports = { STYLE_PROPS, MAX_DEPTH, MAX_CHILDREN, captureDom, staticClasses, normalizeSkeleton, summarizeDomDiff };

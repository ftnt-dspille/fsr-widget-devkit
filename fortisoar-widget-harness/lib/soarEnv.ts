...
interface SoarEnvFileResult extends SoarEnvResult {
  file: string;
}
...
function keyringSecret(account: string, service: string): string {
  if (!account) return "";
  try {
    const { Entry } = require("@napi-rs/keyring") as any;
...
function normalizeHost(raw: string | undefined, port: string | undefined): string {
  raw = (raw || "").trim();
  port = (port || "").trim();
  if (!raw) return "";
...
function resolveSoarEnv(env?: Record<string, string | undefined>, opts?: EnvOpts): SoarEnvResult {
  ...
  function tieredSecret(envVal: string | undefined, fileVal: string | undefined, account: string): string {
...
function resolveSoarEnvFile(envPath: string, opts?: EnvOpts): SoarEnvFileResult {
  const dotenv = require("dotenv") as any;

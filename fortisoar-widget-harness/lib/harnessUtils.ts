...
type StateContext = 'viewpanel' | 'drawer' | 'dashboard' | 'playbook';
...
interface LintOpts {
  ...
  /** Widget-relative paths of every file in the widget dir (for asset-404 check). */
...
/* Faithful-or-loud stub policy (NS2).
   ...
   silently dropped. */
function inertStubFinding(inert: Record<string, number> | null | undefined): string | null {
  if (!inert || typeof inert !== "object") return null;
...
function dollarParamObjectKeys(source: string | null | undefined): string[] {
  if (typeof source !== "string" || !source) return [];
...
function queryFilterMissingLogic(source: string | null | undefined): boolean {
  if (typeof source !== "string" || !source) return false;
  const code = stripJsComments(source);
  if (code.indexOf("/api/query") === -1) return false;
  const re = /\bfilters\s*:/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(code))) {
    const start = Math.max(0, m.index - 500);
    const win = code.slice(start, m.index + 500);
    if (win.indexOf("/api/query") !== -1 && !/\blogic\s*:/.test(win)) return true;
  }
  return false;
}
...
function referencedLocalAssets(html: string | null | undefined): string[] {
  ...
  while ((m = re.exec(stripped))) {
    let p = m[1].trim();
    if (!p || p.startsWith("/") || p.startsWith("//") || /^[a-z]+:/i.test(p) || p.includes("{{")) continue;
    p = p.replace(/^\.\//, "").replace(/[?#].*$/, "");
    out.add(p);
  }
  return Array.from(out);
...
const TEST_BOX_IP = "192.0.2.205";
function absoluteHostUrls(source: string | null | undefined): string[] {
  if (typeof source !== "string" || !source) return [];
  const code = stripJsComments(source);
  const out = new Set<string>();
  const urlRe = /https?:\/\/[^\s"'`)]+/gi;
  let m: RegExpExecArray | null;
  while ((m = urlRe.exec(code))) out.add(m[0]);
  if (code.indexOf(TEST_BOX_IP) !== -1) out.add(TEST_BOX_IP);
  return Array.from(out);
}
...
function generatedServiceNames(model: unknown): string[] {
  ...
  return services
    .map((s) => (s && typeof s === "object" ? (s as { inject?: unknown }).inject : undefined))
...
function htmlTagBalanceErrors(source: string): Array<{ tag: string; opens: number; closes: number }> {
  if (typeof source !== "string" || !source) return [];
  ...
  for (const tag of BALANCED_CONTAINER_TAGS) {
    ...
    if (opens !== closes) out.push({ tag, opens, closes });
  }
  return out;
}
  ...
  for (const f of ["view.html", "edit.html"]) {
    ...
    for (const im of htmlTagBalanceErrors(files[f] || "")) {
      errors.push({
        ...
        file: f,
        message:
          `${f} has unbalanced <${im.tag}> tags: ${im.opens} opening vs ${im.closes} closing. ` +
  ...
  for (const f of ["view.controller.js", "edit.controller.js"]) {
    if (queryFilterMissingLogic(files[f])) {
      warnings.push({
        ...
        file: f,
        message:
          `${f} POSTs to /api/query with a \`filters:\` body but no sibling \`logic:\` key. ` +

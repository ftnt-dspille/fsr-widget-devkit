...
import { chromium, Browser, BrowserContext, Page, Response, ConsoleMessage } from "@playwright/test";
...
function baseUrl(soar: SoarEnvResult): string {
...
interface LaunchContextOpts {
  headless?: boolean;
}

interface BrowserContextPair {
  browser: Browser;
...
async function login(page: Page, base: string, soar: SoarEnvResult): Promise<void> {
 ...
 */
function captureApiErrors(page: Page): CapturedErrors {
 ...
 */
async function openRecord(page: Page, base: string, module: string, uuid: string, { settleMs = 10000 }: OpenRecordOpts = {}): Promise<void> {
...
interface LaunchSoarSessionOpts {
  headless?: boolean;
  env?: Record<string, string | undefined>;
}

interface LaunchSoarSessionResult {
  browser: Browser;
  context: BrowserContext;
  page: Page;
  base: string;
 ...
 */
async function launchSoarSession(opts: LaunchSoarSessionOpts = {}): Promise<LaunchSoarSessionResult> {

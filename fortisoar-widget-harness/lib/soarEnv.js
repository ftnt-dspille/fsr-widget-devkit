...
function resolveSoarEnv(env, opts) {
    ...
    const host = normalizeHost(env.FSR_BASE_URL || env.FORTISOAR_HOST || fileEnv.FSR_BASE_URL || "", env.FSR_PORT || fileEnv.FSR_PORT || "");
    const user = (env.FSR_USERNAME || env.FORTISOAR_USERNAME || fileEnv.FSR_USERNAME || "").trim();
    const pass = tieredSecret(env.FSR_PASSWORD || env.FORTISOAR_PASSWORD || "", fileEnv.FSR_PASSWORD || fileEnv.FORTISOAR_PASSWORD || "", user);
    const apiKey = tieredSecret((env.FSR_API_KEY || "").trim(), (fileEnv.FSR_API_KEY || "").trim(), user ? `${user}:apikey` : "");
...
function resolveSoarEnvFile(envPath, opts) {
    ...
    const service = (opts && opts.service) || KEYRING_SERVICE;
    const host = normalizeHost(fileEnv.FSR_BASE_URL || fileEnv.FORTISOAR_HOST || "", fileEnv.FSR_PORT || "");
    const user = (fileEnv.FSR_USERNAME || fileEnv.FORTISOAR_USERNAME || "").trim();
    const pass = (fileEnv.FSR_PASSWORD || fileEnv.FORTISOAR_PASSWORD || "").trim() ||
        keyringSecret(user, service);
    const apiKey = (fileEnv.FSR_API_KEY || "").trim() ||
...
function isExplicitHostOverride(env, fileEnv) {
    env = env || process.env;

...
| `approval_request` | `context` | UNTRIAGED | **Evidenced on the wire in all five captures, and decision-relevant.** On a `run_playbook` approval it carries the playbook's run history: `last_run` ("21h ago"), `last_status`, `runs_24h`, `runs_total`, plus `playbook_uuid`/`playbook_url`. "This playbook has run 16 times and last finished cleanly" is exactly what an analyst wants before approving a run, and none of it is rendered. Tracker #124. |

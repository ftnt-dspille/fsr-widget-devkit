...
| `patch_proposal` | hermetic-only | fixture-pinned only (`applyPatch`, `applyPatchResume`). A live spec EXISTS (`tests/live/patchProposal.live.test.js`) and does not yet claim the card: its first live run never reached a patch card, because a free-typed change request against an open playbook stopped on the change-affordance gate's "want me to draft the edit?" approval first -- two approvals for one change. That gate is fixed in the framework and the fix is not on a box yet, so the claim stays off and this row stays honest. NOT claimed: that a live patch renders, applies, or that the resumed turn reflects it in the DOM. |

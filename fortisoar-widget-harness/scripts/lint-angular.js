...
function checkNgModelDotRule(file, lines) {
    ...
    lines.forEach((line, i) => {
        ...
        while ((m = re.exec(line)) !== null) {
            ...
            if (/^['"\d]/.test(expr))
                continue;
            record('warning', file, i + 1, 'ng-model-dot-rule', 'ng-model="' + expr + '" binds to a primitive. ng-include / ng-if / ng-repeat ' +
...
function checkConfigDefaultsBeforeAccess(file, lines) {
    ...
    if (guardLine === -1 && accessLines.length > 0) {
        record('warning', file, accessLines[0] + 1, 'config-defaults-missing', 'controller reads $scope.config.X but never guards $scope.config = $scope.config || {}. ' +
            'Cold-mount in a drawer crashes here.');
    }
    else if (guardLine !== -1) {
        accessLines.forEach((i) => {
            if (i < guardLine) {
                record('warning', file, i + 1, 'config-access-before-defaults', 'reads $scope.config before the defaults guard on line ' + (guardLine + 1) + '.');
            }
        });
    }
}
...
function checkUibModalInView(file, lines) {
    lines.forEach((line, i) => {
        if (/\$uibModal\.open\s*\(/.test(line)) {
            record('warning', file, i + 1, 'uibModal-in-view-controller', '$uibModal.open in the view controller is stubbed to a no-op in the harness. ' +
    ...
    lines.forEach((line, i) => {
        ...
        while ((m = re.exec(line)) !== null) {
            ...
            if (registeredControllers.indexOf(name) < 0) {
                record('error', file, i + 1, 'ng-controller-unregistered', 'ng-controller="' + name + '" but no .controller("' + name + '") found in ' +
...
function checkInjectArray(file, lines) {
    ...
    if (!/\.\$inject\s*=\s*\[/.test(src) && !/\.factory\s*\(\s*['"][\w]+['"]\s*,\s*\[/.test(src)) {
        record('warning', file, 1, 'missing-inject', 'controller/factory in this file has no $inject array. Minified builds will fail.');
    }
}
...
function checkEditModalContract(file, lines) {
    ...
    if (!/cancel\s*=\s*function|\$scope\.cancel\s*=/.test(src)) {
        record('warning', file, 1, 'edit-cancel-missing', 'edit.controller.js does not define a cancel() handler.');
    }
}
...
function checkWebsocketCleanup(file, lines) {
    const src = lines.join('\n');
    if (!/websocketService\.subscribe/.test(src))
        return;
    if (!/\$scope\.\$on\s*\(\s*['"]\$destroy['"]/.test(src)) {
        record('warning', file, 1, 'websocket-no-destroy-cleanup', 'websocketService.subscribe() called but no $scope.$on("$destroy", ...) cleanup. ' +
...
function checkDataPrefixOnCsDirectives(file, lines) {
    ...
    lines.forEach((line, i) => {
        ...
        while ((m = re.exec(line)) !== null) {
            ...
            if (/data-$/.test(before))
                continue;
            record('warning', file, i + 1, 'cs-directive-needs-data-prefix', 'use data-' + m[1] + '= instead of bare ' + m[1] + '=. KB §28.4.');
        }
    });
}
...
function checkInfoJson(file, json) {
    ...
    if ('subtitle' in obj && !('subTitle' in obj)) {
        record('error', file, 1, 'subtitle-capitalization', '"subtitle" should be "subTitle" (capital T). KB §28.9.');
    }
    if ('version' in obj && !/^\d+\.\d+\.\d+$/.test(String(obj.version))) {
        record('warning', file, 1, 'version-not-semver', 'version "' + obj.version + '" is not x.y.z. KB §3.0.');
    }
    if ('published_date' in obj) {
        const pd = obj.published_date;
        if (typeof pd === 'string' && /[-:T]/.test(pd)) {
            record('warning', file, 1, 'published-date-iso', 'published_date "' + pd + '" looks like ISO8601. Should be a Unix-seconds integer. KB §28.8.');
        }
    }
    ...
    if (contexts.indexOf('drawer') >= 0 && meta.standalone !== true) {
        record('error', file, 1, 'drawer-needs-standalone', 'metadata.contexts includes "drawer" but metadata.standalone is not true. KB §18.0.');
    }
    ...
    if (!isDrawer && (!Array.isArray(meta.pages) || meta.pages.length === 0)) {
        record('warning', file, 1, 'metadata-pages-empty', 'metadata.pages is empty and this widget is not a drawer. SOAR may block install. ' +
...
function checkConnectorConfigId(file, lines) {
    lines.forEach((line, i) => {
        ...
        if (/executeConnectorAction\s*\([^)]*,\s*null\s*,/.test(line)) {
            record('warning', file, i + 1, 'connector-null-configid', 'executeConnectorAction called with configId=null. The connector will run against ' +
...
function checkWidgetHeightStickyComposer(file, lines) {
    ...
    if (hasBareVh && !hasCalcVh) {
        ...
        record('warning', file, ln + 1, 'widget-root-height-100vh', '`height: 100vh` on the widget root overflows the visible area when the host has ' +
    ...
    if (hasBare100 && !hasCalcVh && !hasBareVh) {
        record('warning', file, 1, 'widget-root-height-100pct', '`height: 100%` on the widget root collapses to content-height in the SOAR drawer ' +
    ...
    if (composerBlock && !/position\s*:\s*sticky/.test(composerBlock[0])) {
        ...
        record('warning', file, ln2 + 1, 'composer-not-sticky', 'composer/input row has no `position: sticky; bottom: 0`. Without it, the input falls ' +
...
function checkCopyrightHeader(file, lines) {
    ...
    if (/Copyright start/i.test(head) && /Copyright end/i.test(head))
        return;
    record('warning', file, 1, 'copyright-header-missing', 'File has no MIT copyright header. Content Hub rejects submissions whose ' +

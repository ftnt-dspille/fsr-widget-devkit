...
/** Per-widget mount override (introspection-profiles.json). Lets drawer/
 *  standalone widgets — which the generic {module:"alerts"} config can't mount —
 *  produce a REAL mounted render (config + context + URL params + a mount probe)
 *  so their stub-hit map and correctness signals are measured, not gated. */
interface IntrospectProfile {
...
function loadProfiles(): Record<string, IntrospectProfile> {
  try {
    const raw = JSON.parse(fs.readFileSync(PROFILES_PATH, "utf8")) as Record<string, unknown>;
    const out: Record<string, IntrospectProfile> = {};
    for (const [k, v] of Object.entries(raw)) {
      if (k !== "_doc" && v && typeof v === "object") out[k] = v as IntrospectProfile;
    }
    return out;
  } catch (_) { return {}; }
}
...
const EDITOR_BUNDLES = [
  { key: "monaco", match: /monaco-editor/i, cap: "monaco" as const },
  { key: "tinymce", match: /tinymce|toastui|dompurify/i, cap: "editors" as const },
];

function classifyResource(name: string, initiatorType: string): ResourceEntry["type"] {
  if (/\.js(\?|$)/.test(name) || initiatorType === "script") return "script";
  if (/\.css(\?|$)/.test(name) || initiatorType === "link") return "css";
  if (/\.(woff2?|ttf|otf|eot)(\?|$)/.test(name)) return "font";
  if (/\.(png|jpe?g|gif|svg|webp|ico)(\?|$)/.test(name)) return "image";
  if (initiatorType === "fetch" || initiatorType === "xmlhttprequest") return "fetch";
  return "other";
}

async function fetchWidgets(browser: Browser): Promise<PublicWidget[]> {
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  const resp = await page.goto(`${HARNESS_URL}/_fsr/widgets`, { waitUntil: "domcontentloaded" });
  const body = (await resp!.json()) as { widgets: PublicWidget[] };
  await ctx.close();
  return body.widgets || [];
}

async function introspectWidget(browser: Browser, widget: PublicWidget): Promise<RenderReport> {
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  ...
  await page.addInitScript((args: { id: string; config: string; ctx: string | null }) => {
    try {
      ...
      if (args.ctx) window.localStorage.setItem("harness.ctx", args.ctx);
    } catch (_) {}
  }, {
    id: widget.id,
    config: JSON.stringify(profile?.config || { module: "alerts" }),
  ...
  const mountState: NonNullable<RenderReport["mountState"]> = probeMounted
    ? "mounted"
    : await page.evaluate(() => {
      const host = document.getElementById("widget-host");
      if (!host) return "no-mount";
      if (host.querySelector(".harness-config-prompt")) return "config-prompt";
      if (host.querySelector(".ng-scope") && host.innerHTML.length > 200) return "mounted";
      return "no-mount";
    });
  ...
  for (const r of resources) r.type = classifyResource(r.name, r.type);
...
function findings(rep: RenderReport): string[] {
  const out: string[] = [];
  ...
  const bySig = new Map<string, ResourceEntry[]>();
  for (const r of rep.resources) {
    if (r.size < 10000) continue;
    const sig = `${path.basename(r.name)}:${r.size}`;
    (bySig.get(sig) || bySig.set(sig, []).get(sig)!).push(r);
  }
  for (const [sig, group] of bySig) {
    if (group.length > 1) out.push(`duplicate fetch ${sig.split(":")[0]} ×${group.length} (${kb(group[0].size)} each) from ${group.length} paths`);
  }
  // Heavy total
  if (rep.totalBytes > 6_000_000) out.push(`heavy render: ${kb(rep.totalBytes)} total across ${rep.resourceCount} resources`);
  // Font weight count
  const fonts = rep.resources.filter((r: ResourceEntry) => r.type === "font");
  if (fonts.length > 3) out.push(`${fonts.length} font files eagerly loaded (${kb(fonts.reduce((s: number, f: ResourceEntry) => s + f.size, 0))})`);
  if (rep.mountState === "config-prompt") out.push(`config-gated: shows "configure to preview" (rig supplied no config — not a failure)`);
  else if (rep.mountState === "no-mount") out.push(`NO RENDER within ${MOUNT_TIMEOUT_MS}ms — real boot/fidelity issue to investigate`);
  if (rep.correctness.errorCount > 0) out.push(`${rep.correctness.errorCount} console error(s) during render`);
  ...
  if (rep.runtime && rep.runtime.stubHits && Object.keys(rep.runtime.stubHits).length > 0) {
    ...
    const stubHits = rep.runtime.stubHits as Record<string, number>;
...
function kb(b: number): string {
  return b >= 1_000_000 ? `${(b / 1_000_000).toFixed(2)} MB` : `${Math.round(b / 1000)} KB`;
}

function diffBaseline(rep: RenderReport): string | null {
  const bpath = path.join(BASELINE_DIR, `${rep.widgetId}.json`);
  if (!fs.existsSync(bpath)) return null;
  try {
    const base = JSON.parse(fs.readFileSync(bpath, "utf8")) as RenderReport;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const w of widgets) {
      ...
      for (const f of findings(rep)) console.log(`    • ${f}`);
      console.log("");
    }

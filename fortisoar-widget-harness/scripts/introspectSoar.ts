...
/** Widgets we know how to render live, and how. Keyed by the harness widget id
 *  so the fidelity diff can pair the soar report with the harness one. */
interface LiveWidget {
  id: string;
  /** drawer icon title (img.logo-sm[title=...]) on the record view. */
  title: string;
  module: string;
  mode: "drawer";
...
async function firstRecordUuid(module: string): Promise<string> {
  ...
  if (!uuid) throw new Error(`no ${module} record on the box to render a widget on`);
  return uuid;
}

/** Render one deployed widget on the box and capture a source:"soar" report. */
async function introspectSoar(lw: LiveWidget): Promise<RenderReport> {
  ...
  if (titled) { await titled.click().catch(() => {}); await page.waitForTimeout(3000); }
...
function soarBrowserEnv(): ReturnType<typeof soarEnv.resolveSoarEnv> {
  return soarEnv.resolveSoarEnv();
}

/** Compare a harness report against the soar report → FidelityDiff. */
function fidelity(harness: RenderReport | null, soar: RenderReport): FidelityDiff {
  const notes: string[] = [];
  const stubbedInHarness: string[] = [];
  ...
  for (const [name, hits] of Object.entries(stubHits)) {
    if (hits > 0) stubbedInHarness.push(name);
  }
...
function loadHarnessReport(id: string): RenderReport | null {
  ...
  matches.sort();
  try { return JSON.parse(fs.readFileSync(matches[matches.length - 1], "utf8")) as RenderReport; }
  catch (_) { return null; }
}

...
async function pickRecentAlertId(ctx, host, user, pass, log) {
    try {
        ...
        if (!authResp.ok()) {
            log(`  auth ${authResp.status()} — cannot query alerts`);
            return null;
        }
        const token = (await authResp.json()).token;
        if (!token) {
            log("  auth returned no token");
            return null;
        }
        const listResp = await ctx.request.get(`${host}/api/3/alerts?$limit=1&$orderby=-createDate`, { headers: { Authorization: `Bearer ${token}` }, timeout: 20000 });
        if (!listResp.ok()) {
            log(`  alerts query ${listResp.status()}`);
            return null;
        }
...
async function run(opts) {
    const { host, user, pass, alert, widgetDir, widgetName, widgetId, outDir, } = opts;
    ...
    page.on("console", (msg) => {
        if (msg.type() !== "error" && msg.type() !== "warning")
            return;
        const text = msg.text();
        if (CONSOLE_IGNORE.some((re) => re.test(text)))
            return;
    ...
    try {
        ...
        if (alert) {
            ...
            if (!alertId)
                throw new Error("API returned no alerts to verify against");
            alertUrl = `${host}/modules/view-panel/alerts/${alertId}`;
        }
        log(`→ alert ${alertUrl}`);
        ...
        await page
            ...
            for (const b of buttons) {
                ...
                if (r.x > vw - 80 && r.width < 80 && r.height < 80 && r.width > 0)
                    return true;
            }
            return false;
        }, { timeout: 45000 })
        ...
        if (!drawerOpened)
            throw new Error(`widget root ${rootSelector} did not render after clicking any drawer trigger (tried: ${triggerSelectors.join(" | ")})`);

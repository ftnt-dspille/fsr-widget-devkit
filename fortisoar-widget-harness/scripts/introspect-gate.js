...
function runGate() {
    ...
    const inScope = (widgetId) => !filter || widgetId === filter || widgetId.startsWith(filter + "-");
    ...
    for (const { widgetId, report, baseline } of reports) {
        ...
        if (payloadDeltaPct > THRESHOLDS.payloadDeltaPercent) {
            checks.push(`payload ${(payloadDeltaPct > 0 ? "+" : "") + payloadDeltaPct.toFixed(1)}% ` +
        ...
        if (bootDeltaPct > THRESHOLDS.bootDeltaPercent) {
            checks.push(`boot DCL ${(bootDeltaPct > 0 ? "+" : "") + bootDeltaPct.toFixed(1)}% ` +
        ...
        if (reportErrors > baselineErrors) {
            checks.push(`console errors increased from ${baselineErrors} to ${reportErrors}`);
        }

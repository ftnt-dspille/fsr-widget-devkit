...
exports.writeWidget = writeWidget;
const fs = require("fs");
...
function normalizeSpec(raw) {
    ...
    return {
        ...
        description: raw.description || "What this widget renders and how it's configured.",
        category: raw.category || "Utilities",
        publisher: raw.publisher || "Your Team",
        compatibility: Array.isArray(raw.compatibility) && raw.compatibility.length
            ? raw.compatibility
            : ["7.6.0"],
...
const COPYRIGHT = `/* Copyright start
   ...
   Copyright end */`;
function buildInfoJson(spec) {
    const pages = spec.kind === "record" ? ["View Panel"] : ["Dashboard", "View Panel"];
    const info = {
        name: spec.name,
        title: spec.title,
...
function buildViewController(spec) {
...
`;
    ...
    const inject = ['"$scope"', '"config"'];
    const args = ["$scope", "config"];
    if (spec.kind === "record") {
...
function buildEditController(spec) {
    const digits = versionDigits(VERSION);
...
`;
    ...
    const makeCtor = spec.kind === "record"
...
${spec.kind === "record" ? "    FormEntityService = _$injector_.get(\"FormEntityService\");\n" : ""}${spec.triggersPlaybook
        ...
        : ""}    _$controller_(CTRL_NAME, ${spec.triggersPlaybook
        ? "{ $scope, config: config || {}" +
...
function buildE2eSpec(spec) {
    ...
    let assertion = spec.kind === "record"
...
// ─── orchestration ────────────────────────────────────────────────────────────
function generateWidget(rawSpec) {
...
function parseCli(argv) {
    ...
    if (positional[0])
        spec.name = positional[0];
    if (positional[1])
        spec.title = positional[1];
    // Inline flags override the spec file.
    if (flags.name)
        spec.name = String(flags.name);
    if (flags.title)
        spec.title = String(flags.title);
    if (flags.subtitle)
        spec.subTitle = String(flags.subtitle);
    if (flags.description)
        spec.description = String(flags.description);
    if (flags.category)
        spec.category = String(flags.category);
    if (flags.publisher)
        spec.publisher = String(flags.publisher);
    if (flags.compat)
        spec.compatibility = String(flags.compat).split(",").map((s) => s.trim());
    if (flags.kind)
        spec.kind = flags.kind === "record" ? "record" : "dashboard";
...
function main() {
    ...
    const widgetsDir = process.env.WIDGETS_SRC || path.join(repoDir, "widgets-src");
    ...
    process.stdout.write(`✓ created ${result.dest}\n` +

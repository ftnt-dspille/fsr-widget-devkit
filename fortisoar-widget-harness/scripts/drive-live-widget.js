...
(async () => {
  const recordUuid = arg("record");
  if (!recordUuid) {
    console.error("error: --record <uuid> is required");
    process.exit(2);
  }
  const module = arg("module", "alerts");
  const message = arg("message", "What is the severity of this alert?");
  const headless = !arg("headed", false);
  const shot = arg("shot", "/tmp/live-widget.png");

  console.log(`[drive] module=${module} record=${recordUuid} headless=${headless}`);
  const s = await openWidgetDrawer({ module, recordUuid, headless });
  ...
  console.error("[drive] error:", e && e.message);
  process.exit(1);
});

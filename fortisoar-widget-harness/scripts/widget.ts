...
function resolveLocalWidget(): void {
...
function http(urlStr: string, opts: HttpOpts = {}, body: string | null = null): Promise<HttpResponse> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = mod(u, reqOpts, (res: any) => {
      const chunks: Buffer[] = [];
...
async function harnessAlive(): Promise<boolean> {
  try {
...
function info_(msg: string): void {
  console.log("·", msg);
}
...
function prompt(question: string, def?: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    rl.question(def ? `${question} [${def}]: ` : `${question}: `, (a: string) => {
      rl.close();
      resolve((a || "").trim() || def || "");
    });
  });
...
interface MutableReadlineInterface extends readline.Interface {
  stdoutMuted?: boolean;
...
async function cmdBump(): Promise<void> {
  ...
  }, JSON.stringify({ bump })).catch((): null => null);
  if (!r) die("bump request failed");
  if (r.status >= 400) die(`bump failed (${r.status}): ${r.text.slice(0, 400)}`);
  ok(`bumped ${folderName} → ${(r.json && (r.json as Record<string, unknown>).version) || "(see harness log)"}`);
}
...
async function cmdPush(): Promise<void> {
  ...
  const rJson = r.json as Record<string, unknown> | undefined;
...
async function cmdVerifyRemote(): Promise<void> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- dynamic require result
  const verifyMod: any = require("./widget-verify-remote");
...
async function cmdList(): Promise<void> {
  ...
  const rows = ((r.json as any).widgets || []).map(
...
async function cmdRemoteList(): Promise<void> {
...
async function cmdPull(): Promise<void> {
  ...
  if (!/^[a-f0-9-]{16,}$/i.test(idArg)) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- response JSON shape is dynamic
    const matches = ((lr.json as any).widgets || []).filter(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- dynamic response item shape
...
async function cmdLint(): Promise<void> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- response JSON shape is dynamic
  const { errors = [], warnings = [] } = ((r.json as any).lint || {});
...
function printTable(headers: string[], rows: Array<(string | number | undefined)[]>): void {
  const widths = headers.map((h, i) =>
    Math.max(h.length, ...rows.map((r) => String(r[i] == null ? "" : r[i]).length))
  );
  const fmt = (cells: (string | number | undefined)[]): string => cells.map((c, i) => String(c == null ? "" : c).padEnd(widths[i])).join("  ");
  console.log(fmt(headers as unknown as (string | number | undefined)[]));

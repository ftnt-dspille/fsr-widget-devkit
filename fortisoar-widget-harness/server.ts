...
const resolveAppUnmin = (dir: string): string =>
  [path.join(dir, "app.unmin.js"), path.join(dir, "app_min", "app.unmin.js")].find((p) =>
    fs.existsSync(p)
  ) || path.join(dir, "app.unmin.js");
...
app.post("/_fsr/fix-info/:id", express.json(), (req: express.Request, res: express.Response) => {
  ...
  if (bump && typeof bump === "string") {
    ...
    try {
      ...
      const newVersion = bumpVersion(info.version, bump as "patch" | "minor" | "major");
...
const executeLocal = async (req: express.Request, res: express.Response): Promise<void> => {
  try {
...
app.post("/api/integration/execute/", (req, res, next) => {
  if (LOCAL_CONNECTOR) { void executeLocal(req, res); return; }

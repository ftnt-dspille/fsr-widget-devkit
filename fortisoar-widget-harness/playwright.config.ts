...
function realWidgetsSrc(): string | null {
  try {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- playwright config is dynamic
const projects: any[] = [
  {
    name: "chromium",
    testDir: __dirname,
...
if (widgetsSrc) {
  projects.push({
    name: "widgets",
    testDir: widgetsSrc,
    testMatch: ["*/tests/e2e/**/*.spec.js", "*/tests/e2e/**/*.spec.ts"],
    use: sharedUse,
  });
}
...
export default defineConfig({
  ...
  // runs. See tests/e2e/_hermeticTeardown.ts.
  globalTeardown: require.resolve("./tests/e2e/_hermeticTeardown.ts"),
  ...
  testIgnore: process.env.E2E_LIVE
    ? []
    : ["**/*[Ll]ive*.spec.js", "**/*[Ll]ive*.spec.ts"],
  timeout: 45000,
  ...
  // _isolated.ts baseURL fixture. Live runs go serial against the real box.
  ...
  // share a Node process. parallelIndex → port mapping lives in _isolated.ts.
  webServer: [14401, 14402].map((port) => ({
    ...
    env: {
      FSR_BASE_URL:
        process.env.FSR_BASE_URL ||
        process.env.FORTISOAR_HOST ||
        "https://soar.test.invalid",
      FSR_USERNAME:
        process.env.FSR_USERNAME ||
        process.env.FORTISOAR_USERNAME ||
        "admin",
      FSR_PASSWORD:
        process.env.FSR_PASSWORD ||
        process.env.FORTISOAR_PASSWORD ||
        "test",

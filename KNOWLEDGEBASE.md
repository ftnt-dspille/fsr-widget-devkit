...
> **Gotcha -- `SetVariable` values are referenced as `{{ vars.<name> }}`, NOT `{{ vars.steps.<step_name>.<name> }}`.** A runner playbook that set variables in a `SetVariable` step named `set_run_vars` and then referenced them in a later `Connectors` step via `{{ vars.steps.set_run_vars.session_id }}` saw blank values at execution time -- the connector received empty strings. The correct reference is `{{ vars.session_id }}` (by variable name, not by step path). The `vars.steps.*` path silently resolves to empty rather than erroring, so the wrong syntax looks like a platform bug when it's a reference-path mistake. **Fix:** use `{{ vars.<variable_name> }}` to read SetVariable outputs in later steps. Alternatively, bake literal values (and `{{ uuid() }}` for fresh IDs) directly into each `Connectors` step's `params` block -- the runner does this for simplicity, since the values are static per task. If a downstream step needs a value from an upstream step (e.g. `notify_finding` needs the `session_id` from `chat_turn`), have the connector read it from storage by task name instead of passing it through Jinja. See `connector-fsr-soc-assistant/operations.py` `_compile_runner_playbook()`.

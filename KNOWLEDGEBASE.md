...
> **Gotcha -- `SetVariable` + `{{ vars.steps.<name>.<field> }}` inter-step Jinja references resolve to **empty** on FortiSOAR 8.0.** A runner playbook that sets variables in a `SetVariable` step and then references them in a later `Connectors` step via `{{ vars.steps.set_run_vars.session_id }}` sees blank values at execution time -- the connector receives empty strings, not the computed values. The platform's Jinja resolution for inter-step variable references is unreliable for dynamically-generated playbooks. **Fix:** bake literal values (and `{{ uuid() }}` for fresh IDs) *directly* into each `Connectors` step's `params` block -- don't route through a `SetVariable` intermediary. If a downstream step needs a value from an upstream step (e.g. `notify_finding` needs the `session_id` from `chat_turn`), have the connector read it from storage by task name instead of passing it through Jinja. See `connector-fsr-soc-assistant/operations.py` `_compile_runner_playbook()` and `tests/test_runner_connector_name.py`.

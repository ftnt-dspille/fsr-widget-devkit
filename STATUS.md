...
| **harness `tests/server.test.ts` — symptom gone, now FLAKY under load** | Re-checked 2026-07-23. The documented failure is **gone**: the `GET /<widget-id>/view.html` 404 no longer reproduces, 30/30 pass standalone and root `make test-unit` is clean (15 suites / 424 passed). BUT the suite is slow (15–22s) and failed **once in three** full `make test-unit WIDGET=fortiaiAgenticAssistant` runs, where it competes with the widget project for workers — i.e. a load/timeout flake, not a stale assertion. Next action: raise this suite's jest timeout or serialize it (it boots a real server), then close the row. Do NOT re-title it "green" — an intermittent red is worse than a steady one. | none | `fortisoar-widget-harness/tests/server.test.ts` |
...
| `playbook-compiler-fidelity-and-agent-surface.md` | 🆕 **2026-07-22, spec only** — round-trip fidelity gate + agent-facing compiler errors. _Proposed 2026-07-21, deprioritized, never written down until now._ |

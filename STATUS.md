...
_Last updated: 2026-07-13 (C1 done; live-test session started)_
...
> - **C2 PUT endpoint** (`PUT /api/3/workflows/<uuid>` bare-workflow body) is the
>   committed-but-never-live-verified detail. A no-ship pyfsr probe
>   (`scripts/_c2_put_live_confirm.py`, gitignored, self-cleaning) is written +
>   ready to run next session (two bugs found+fixed: envelope unwrap, recycle-bin
>   hard-delete). Box verified clean of test litter.

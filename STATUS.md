...
| ~~harness `tests/server.test.ts` red (pre-existing)~~ | **STALE — re-verified green 2026-07-23.** `tests/server.test.ts` passes 30/30 (incl. the `GET /<widget-id>/view.html` case that used to 404), and root `make test-unit` is clean: **14 suites / 411 passed**. Fixed at some point without this row being updated; no action left. | — | `fortisoar-widget-harness/tests/server.test.ts` |

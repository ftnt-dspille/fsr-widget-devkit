...
| **harness `tests/server.test.ts` red (pre-existing)** | `GET /<widget-id>/view.html` returns 404 for widget-jinja-editor; expects 200. Predates 4f and is unrelated to it, but it means `make test-unit` at repo root is not clean. | none | `fortisoar-widget-harness/tests/server.test.ts` |

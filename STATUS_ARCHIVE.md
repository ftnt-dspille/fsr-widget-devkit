...
> (198.51.100.206, 10 workers recycled, warmup 21 conn/283 ops, commit `7ed859b`) → widget **1.2.36**
...
> - 📦 **Also deployed to .206** (198.51.100.206, user request): **connector 0.5.11** (BUMP=none, 10 workers,
...
> GA = 198.51.100.159:13000. **Beats 1-4 (open record → investigate → enrich → verdict) are
...
> - **SHIPPED the full stack to 159** (`fsr8`, 198.51.100.159, 8.0 GA -- **206 still down**, 168
>   lacks `/api/ai/*`):
...
>   ship-verify). `fsr8`/`fsr159` = **198.51.100.159** (not .250.159). Framework pre-commit
...
>     deploy confirmed to 198.51.100.206).
...
> - **GA/159 (`.env.fsr-ga` = 198.51.100.159:13000): READY.** FortiAI enabled,
...
>   reachable.
> - **168 (198.51.100.168): native FortiAI ABSENT** -- `/api/ai/*` routes 404 (older
